...
test('Unlike Community Post Controller', function ({ components, stubComponents }) {
  ...
  describe('when listing posts after unliking', () => {
    let ownerIdentity: Identity
    let memberIdentity: Identity
    let communityId: string
    let postId: string
    ...
    describe('and the user unlikes the post', () => {
      it('should update the likes count and isLikedByUser status', async () => {
        // Unlike the post
        await makeRequest(memberIdentity, `/v1/communities/${communityId}/posts/${postId}/like`, 'DELETE')
