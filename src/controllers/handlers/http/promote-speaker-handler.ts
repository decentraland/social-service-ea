...
export async function promoteSpeakerHandler(
  context: Pick<
    HandlerContextWithPath<'logs' | 'communities' | 'commsGatekeeper' | 'communitiesDb', '/v1/communities/:id/voice-chat/promote-speaker'>,
  ...
  try {
    ...
    // Check if user has permission to promote speakers (must be moderator or owner)
    if (userRole !== CommunityRole.Moderator && userRole !== CommunityRole.Owner) {
      return {
        status: 403,
        body: {
          message: 'You do not have permission to promote speakers'
        }
      }
    }
    ...
    // Send request to comms-gatekeeper to promote user
    ...
    logger.info('User promoted to speaker in community voice chat', {
      communityId,
      targetUserAddress,
    ...
    logger.error('Error handling promote speaker', { error: errorMessageOrDefault(error), communityId, userAddress })
