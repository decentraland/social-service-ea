...
export async function kickPlayerHandler(
  context: Pick<
    HandlerContextWithPath<'logs' | 'communities' | 'commsGatekeeper' | 'communitiesDb', '/v1/communities/:id/voice-chat/kick-player'>,
  ...
  try {
    ...
    // Check if user has permission to kick players (must be moderator or owner)
    ...
    // Send request to comms-gatekeeper to kick user
    await commsGatekeeper.kickUserFromCommunityVoiceChat(communityId, targetUserAddress)

    logger.info('User kicked from community voice chat', {
      communityId,
      targetUserAddress,
      kickedBy: userAddress
    })
    ...
    logger.error('Error handling kick player', { error: errorMessageOrDefault(error), communityId, userAddress })
